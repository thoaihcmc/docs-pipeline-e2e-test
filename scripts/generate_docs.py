...
def _call_openai(manifest: dict, repo_path: str) -> dict:
    ...
    user = (
        ...
        "Include database operations (CRUD, schema init) if SQL or ORM usage is found in any file.\n"
        "2. BDD (Business Design Document): describe the business logic and processes from ALL source files. "
        "Include data persistence and storage logic if any file contains SQL, ORM, or database calls.\n"
        "3. ADD (Architecture Design Document): describe the system architecture covering ALL modules/components. "
        "Include a Database Schema section: scan ALL evidence files for CREATE TABLE, ALTER TABLE, sqlite3, "
        "SQLAlchemy models, Django models, or any ORM/SQL — even inside string literals or executescript() calls. "
        "Document every table, its columns, types, constraints, and foreign-key relationships. "
        "Only write 'No database evidence' if zero SQL/ORM patterns exist in ANY file.\n"
